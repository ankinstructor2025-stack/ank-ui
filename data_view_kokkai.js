...
window.DataViewKokkai = (() => {
  ...
  function renderChildTable(rows, parentRow) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      ctx.renderChildPlaceholder("子データがありません。");
  ...
  function bindParentRowEvents() {
    ...
    rows.forEach((tr) => {
      tr.addEventListener("click", async () => {
        ...
        ctx.renderChildPlaceholder("子一覧を読み込み中です...");
        ctx.detailPre.textContent = `選択中: ${formatParentLabel(row)}\n子一覧を読み込み中です...`;
        ...
        try {
          ...
          console.error(e);
          ctx.renderChildPlaceholder(e.message);
    ...
    ctx.parentTableBody.innerHTML = rows.map((row, index) => {
      return `
        <tr class="clickable-row parent-row" data-index="${index}">
          ...
          <td><span class="${ctx.getStatusClass(row.status)}">${ctx.escapeHtml(row.status ?? "")}</span></td>
        </tr>
      `;
    }).join("");
