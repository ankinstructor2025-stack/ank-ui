...
const demoResults = [
  {
    ...
    answer: "政府としては国民負担や電力価格への影響を踏まえつつ、制度全体の見直しを検討している。",
    content: ""
  },
  {
    entry_id: "e002",
    ...
    question: "",
    answer: "",
    content: "制度開始以降、再生可能エネルギー導入量は増加した一方で、国民負担とのバランスが継続的な論点となっている。"
  },
  {
    entry_id: "e003",
    knowledge_type: "qa",
...
function updateDatabaseInfo() {
  const db = demoDatabases.find((item) => item.db_name === currentDatabase);

  if (!db) {
    databaseInfo.textContent = "データベースを選択してください。";
    return;
  }

  databaseInfo.textContent =
    `build: ${db.built_at} / total: ${db.entry_count} / qa: ${db.qa_count} / plain: ${db.plain_count}`;
}
